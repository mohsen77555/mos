...
const DB = {
  ...
  defaults() {
    return {
      ...
      projects: [],   // المشاريع
      timesheets: [], // ساعات العمل المسجَّلة
      settings: {
        company: 'شركتي',
        ...
        statement: null,         // كشف الحساب المستورد للتسوية البنكية { accId, lines }
      },
    };
  },
...
const ROLE_APPS = {
  admin: '*',
  accountant: ['dashboard', 'partners', 'products', 'sales', 'purchases', 'inventory', 'manufacturing', 'invoicing', 'subscriptions', 'treasury', 'accounting', 'payroll', 'projects', 'assets', 'reports', 'governance', 'guide'],
...
const PROJECT_STATUS = { planned: 'مخطّط', active: 'جارٍ', onhold: 'متوقّف', done: 'منجز' };
...
const Models = {
  ...
  projects: {
    label: 'مشروع', plural: 'المشاريع', icon: '📋', color: '#0891b2', menu: false,
    fields: [
      { name: 'name', label: 'اسم المشروع *', type: 'text', required: true },
      { name: 'partnerId', label: 'العميل', type: 'select', required: false, options: () => [{ v: '', l: '— بلا عميل —' }].concat(DB.list('partners').filter(p => p.kind !== 'vendor').map(p => ({ v: p.id, l: p.name }))) },
      { name: 'status', label: 'الحالة', type: 'select', options: kv(PROJECT_STATUS), default: 'active' },
      { name: 'budget', label: 'الموازنة', type: 'number', default: 0 },
      { name: 'startDate', label: 'تاريخ البدء', type: 'date', default: () => todayISO() },
      { name: 'note', label: 'وصف', type: 'textarea' },
    ],
  },
...
const APPS = [
  ...
  { route: 'projects', label: 'المشاريع', icon: '📋', color: '#0891b2' },
...
const App = {
  ...
  fabRoutes: { partners: 1, products: 1, sales: 1, purchases: 1, employees: 1, accounting: 1, crm: 1, assets: 1, projects: 1 },
...
/* ---------------------------------------------------------------------
   7و) المشاريع وساعات العمل (Timesheets)
   --------------------------------------------------------------------- */
function projectEntries(pid) { return DB.list('timesheets').filter(t => t.projectId === pid); }
function projectHours(pid) { return +projectEntries(pid).reduce((s, t) => s + num(t.hours), 0).toFixed(2); }
function projectBillable(pid) { return +projectEntries(pid).reduce((s, t) => s + num(t.hours) * num(t.rate), 0).toFixed(2); }

/* إنشاء أمر بيع مسودة من ساعات المشروع القابلة للفوترة */
function createInvoiceFromProject(pid) {
  const pr = DB.get('projects', pid);
  if (!pr) return null;
  if (!pr.partnerId) { toast('حدِّد عميل المشروع أولاً (تعديل المشروع)'); return null; }
  const amount = projectBillable(pid);
  const hours = projectHours(pid);
  if (amount <= 0) { toast('لا توجد ساعات قابلة للفوترة'); return null; }
  const so = DB.upsert('sales', {
    ref: DB.nextRef('SO'), partnerId: pr.partnerId, date: todayISO(),
    status: 'draft', paid: 0, currency: 'BASE', rate: 1,
    note: 'فاتورة مشروع: ' + pr.name, projectId: pid,
    lines: [{ productId: '', name: `ساعات «${pr.name}»`, qty: hours, price: +(amount / hours).toFixed(4), disc: 0 }],
  });
  Audit.log('create', 'sales', so.ref, 'فاتورة من مشروع');
  toast('تم إنشاء أمر بيع مسودة من المشروع ✅');
  App.go('sales');
  return so;
}

/* حوار تسجيل ساعات عمل */
function openTimesheetDialog(pid) {
  const pr = DB.get('projects', pid);
  if (!pr) return;
  const emps = DB.list('employees');
  const empOpts = emps.map(e => `<option value="${e.id}">${esc(e.name)}</option>`).join('');
  document.getElementById('modalTitle').textContent = 'تسجيل ساعات — ' + pr.name;
  document.getElementById('modalForm').innerHTML = `
    <div class="field"><label>الموظف</label><select name="employeeId">${empOpts || '<option value="">— لا يوجد —</option>'}</select></div>
    <div class="field"><label>التاريخ</label><input name="date" type="date" value="${todayISO()}" required /></div>
    <div class="field"><label>عدد الساعات</label><input name="hours" type="number" inputmode="decimal" step="any" min="0" required /></div>
    <div class="field"><label>سعر الساعة</label><input name="rate" type="number" inputmode="decimal" step="any" min="0" value="0" /></div>
    <div class="field"><label>الوصف</label><input name="note" placeholder="المهمة المنجزة" /></div>
    <button type="submit" class="btn-primary">تسجيل</button>`;
  document.getElementById('modal').classList.remove('hidden');
  document.getElementById('modalForm').onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    if (num(fd.get('hours')) <= 0) { toast('أدخل عدد ساعات صحيح'); return; }
    DB.upsert('timesheets', {
      projectId: pid, employeeId: fd.get('employeeId'), date: fd.get('date'),
      hours: num(fd.get('hours')), rate: num(fd.get('rate')), note: (fd.get('note') || '').trim(),
    });
    Audit.log('create', 'timesheets', pr.name, `${fd.get('hours')} ساعة`);
    closeForm();
    document.getElementById('modalForm').onsubmit = submitForm;
    toast('تم تسجيل الساعات ✅');
    App.render();
  };
}

/* قائمة سجل ساعات مشروع */
function openTimesheetsList(pid) {
  const pr = DB.get('projects', pid);
  if (!pr) return;
  const list = projectEntries(pid).slice().sort((a, b) => String(b.date).localeCompare(String(a.date)));
  document.getElementById('modalTitle').textContent = 'سجل الساعات — ' + pr.name;
  const rows = list.length ? list.map(t => `<div class="rep-row">
      <span><b>${fmtQty(t.hours)}</b> ساعة • ${esc((DB.get('employees', t.employeeId) || {}).name || '—')}<br>
        <span class="muted-text">${fmtDate(t.date)}${t.note ? ' • ' + esc(t.note) : ''}</span></span>
      <span>${fmtMoney(num(t.hours) * num(t.rate))} <button class="del" data-ts-del="${t.id}" style="border:none;background:none;cursor:pointer">🗑️</button></span>
    </div>`).join('') : '<div class="muted-text">لا توجد ساعات مسجَّلة.</div>';
  document.getElementById('modalForm').innerHTML = rows +
    `<div class="rep-row strong" style="margin-top:8px"><span>الإجمالي</span><span>${fmtQty(projectHours(pid))} ساعة • ${fmtMoney(projectBillable(pid))}</span></div>
     <button type="button" class="btn-primary" id="tsCloseBtn" style="margin-top:10px">إغلاق</button>`;
  document.getElementById('modal').classList.remove('hidden');
  document.getElementById('modalForm').onsubmit = (e) => e.preventDefault();
  document.querySelectorAll('[data-ts-del]').forEach(b => {
    b.onclick = () => { DB.remove('timesheets', b.dataset.tsDel); openTimesheetsList(pid); };
  });
  document.getElementById('tsCloseBtn').onclick = () => { closeForm(); document.getElementById('modalForm').onsubmit = submitForm; App.render(); };
}
...
const Views = {
  ...
  /* ===== دليل الاستخدام ===== */
  /* ===== المشاريع وساعات العمل ===== */
  projects() {
    const list = DB.list('projects').slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    const items = filterBySearch(list, ['name', 'note']);
    let html = searchBar('ابحث في المشاريع...');
    const totalBillable = list.reduce((s, p) => s + projectBillable(p.id), 0);
    const totalHours = list.reduce((s, p) => s + projectHours(p.id), 0);
    html += `<div class="stat-grid">
      <div class="stat-card" style="--c:#0891b2"><span class="ico">📋</span><div class="num">${list.length}</div><div class="lbl">المشاريع</div></div>
      <div class="stat-card" style="--c:#6610f2"><span class="ico">⏱️</span><div class="num">${fmtQty(totalHours)}</div><div class="lbl">إجمالي الساعات</div></div>
      <div class="stat-card" style="--c:#198754"><span class="ico">💰</span><div class="num">${fmtMoney(totalBillable)}</div><div class="lbl">قابل للفوترة</div></div>
    </div>`;
    if (!items.length) return html + emptyState('📋', 'لا توجد مشاريع', 'اضغط «＋» لإنشاء مشروع.');
    const stCls = { planned: 'muted', active: 'info', onhold: 'warn', done: 'ok' };
    items.forEach(p => {
      const hours = projectHours(p.id), bill = projectBillable(p.id), budget = num(p.budget);
      const pct = budget > 0 ? Math.min(100, Math.round(bill / budget * 100)) : 0;
      html += `<div class="card"><div class="row"><div>
          <div class="title">${esc(p.name)} <span class="badge ${stCls[p.status] || 'muted'}">${PROJECT_STATUS[p.status] || ''}</span></div>
          <div class="meta">${p.partnerId ? 'العميل: ' + esc(partnerName(p.partnerId)) + ' • ' : ''}بدأ ${fmtDate(p.startDate)}</div>
          <div class="meta">⏱️ ${fmtQty(hours)} ساعة • 💰 ${fmtMoney(bill)}${budget > 0 ? ' / موازنة ' + fmtMoney(budget) : ''}</div>
        </div></div>
        ${budget > 0 ? `<div class="progress"><div class="progress-bar ${bill > budget ? 'over' : ''}" style="width:${pct}%"></div></div>` : ''}
        <div class="card-actions">
          <button data-ts-add="${p.id}">⏱️ تسجيل ساعات</button>
          <button data-ts-list="${p.id}">📋 السجل</button>
          ${p.partnerId && bill > 0 ? `<button data-ts-invoice="${p.id}">🧾 إنشاء فاتورة</button>` : ''}
          <button data-edit="projects:${p.id}">✏️</button>
          <button class="del" data-del="projects:${p.id}">🗑️</button>
        </div></div>`;
    });
    return html;
  },
  ...
  treasury() {
    ...
    if (App.treasTab === 'reconcile') {
      ...
      html += `<div class="card-actions" style="margin-top:0">
        <button class="mini-btn" id="importStmtBtn">📥 استيراد كشف حساب (CSV)</button>
        ${DB.data.settings.statement && DB.data.settings.statement.accId === App.recAcc ? '<button class="mini-btn del" id="clearStmtBtn">🗑️ مسح الكشف</button>' : ''}
      </div>
      <div class="meta" style="margin:6px 0 10px">صيغة الكشف: التاريخ، الوصف، المبلغ (+ إيداع / − سحب). تُطابَق الأسطر تلقائياً مع القيود بالمبلغ.</div>`;
      ...
      // ملخّص كشف الحساب المستورد (إن وُجد لهذا الحساب)
      const stmt = DB.data.settings.statement;
      if (stmt && stmt.accId === App.recAcc) {
        const stmtBal = num(acc ? acc.opening : 0) + stmt.lines.reduce((s, l) => s + num(l.amount), 0);
        const mt = stmt.lines.filter(l => l.matched).length;
        const unmatched = stmt.lines.filter(l => !l.matched);
        html += `<div class="card report-table" style="border:2px solid var(--info)">
          ${reportRow('أسطر الكشف', stmt.lines.length)}
          ${reportRow('مُطابَقة تلقائياً', mt)}
          ${reportRow('غير مُطابَقة', unmatched.length)}
          ${reportRow('رصيد الكشف', fmtMoney(stmtBal), true)}
          ${reportRow('فرق الدفتري عن الكشف', fmtMoney(book - stmtBal))}</div>`;
        if (unmatched.length) {
          html += `<div class="section-title">أسطر الكشف غير المُطابَقة</div>`;
          unmatched.forEach(l => {
            html += `<div class="card"><div class="row"><div>
                <div class="title">${esc(l.desc || '—')}</div>
                <div class="meta">${l.date ? fmtDate(l.date) : 'بلا تاريخ'} • ليست في الدفاتر</div>
              </div><span class="badge ${num(l.amount) >= 0 ? 'ok' : 'danger'}">${num(l.amount) >= 0 ? '+' : ''}${fmtMoney(l.amount)}</span></div></div>`;
          });
        }
      }
...
function docCard(coll, d) {
  ...
  const linesHtml = (d.lines || []).map(l =>
    `<div class="line-row"><span>${esc(l.name || productName(l.productId))} × ${fmtQty(l.qty)}${num(l.disc) ? ` <span class="badge muted">-${num(l.disc)}%</span>` : ''}</span><span>${fmtDoc(lineNet(l), d)}</span></div>`
  ).join('');
...
function openForm(coll, id = null) {
  ...
  const titles = {
    ...
    projects: id ? 'تعديل مشروع' : 'مشروع جديد',
    assets: id ? 'تعديل أصل' : 'أصل ثابت جديد',
  };
...
function printDoc(coll, id) {
  ...
  const rows = (doc.lines || []).map((l, i) => `
    <tr><td>${i + 1}</td><td>${esc(l.name || productName(l.productId))}</td>
...
function bindViewEvents() {
  ...
  on('[data-del]', 'click', b => {
    ...
    if (confirm('هل تريد الحذف نهائياً؟')) {
      ...
      if (c === 'projects') DB.data.timesheets = DB.list('timesheets').filter(t => t.projectId !== i);   // حذف ساعات المشروع
      DB.remove(c, i);
  ...
  // المشاريع وساعات العمل
  on('[data-ts-add]', 'click', b => openTimesheetDialog(b.dataset.tsAdd));
  on('[data-ts-list]', 'click', b => openTimesheetsList(b.dataset.tsList));
  on('[data-ts-invoice]', 'click', b => createInvoiceFromProject(b.dataset.tsInvoice));
  ...
  const importStmt = document.getElementById('importStmtBtn');
  if (importStmt) importStmt.onclick = () => importBankStatement(App.recAcc);
  const clearStmt = document.getElementById('clearStmtBtn');
  if (clearStmt) clearStmt.onclick = () => { DB.data.settings.statement = null; DB.save(); toast('تم مسح الكشف'); App.render(); };
...
/* تحليل سطر كشف حساب: التاريخ + الوصف + المبلغ (موقّع) */
function parseStatementRow(cells) {
  let date = '', desc = '', amount = null;
  cells.forEach(c => {
    const s = String(c).trim();
    if (!s) return;
    if (!date && /^\d{4}[-/]\d{1,2}[-/]\d{1,2}/.test(s)) { date = s.slice(0, 10).replace(/\//g, '-'); return; }
    const n = Number(s.replace(/[,٬\s]/g, ''));
    if (amount === null && /\d/.test(s) && !isNaN(n)) { amount = n; return; }
    desc = desc ? desc + ' ' + s : s;
  });
  return { date, desc, amount };
}

/* استيراد كشف بنكي (CSV) ومطابقته تلقائياً مع قيود الحساب */
function importBankStatement(accId) {
    ...
    reader.onload = () => {
      try {
        ...
        rows.forEach(r => {
          ...
          if (p.amount === null) return;   // سطر بلا مبلغ (ترويسة مثلاً)
          lines.push({ date: p.date, desc: p.desc, amount: +p.amount.toFixed(2), matched: false, journalId: '' });
        });
        if (!lines.length) { toast('لا توجد أسطر صالحة في الكشف'); return; }
        const rmap = DB.data.settings.reconciled || (DB.data.settings.reconciled = {});
        const cands = [];
        DB.list('journal').forEach(j => (j.lines || []).forEach(l => {
          if (l.accountId === accId) cands.push({ id: j.id, amt: +(num(l.debit) - num(l.credit)).toFixed(2) });
        }));
        const used = {};
        let matched = 0;
        lines.forEach(sl => {
          const cand = cands.find(c => !used[c.id] && Math.abs(c.amt - sl.amount) < 0.01);
          if (cand) { used[cand.id] = true; rmap[cand.id] = true; sl.matched = true; sl.journalId = cand.id; matched++; }
        });
        DB.data.settings.statement = { accId, importedAt: Date.now(), lines };
        DB.save();
        Audit.log('update', 'settings', 'تسوية بنكية', `استيراد كشف ${lines.length} سطر — طُوبق ${matched}`);
        toast(`تم استيراد ${lines.length} سطر، وطُوبق ${matched} تلقائياً ✅`);
        App.render();
      } catch (e) { alert('تعذّر قراءة الكشف: ' + e.message); }
    };
